...
static void * (*tr_old_memalign_hook) (size_t __alignment, size_t __size, __const void *);
     ...
     const void * caller;
{
 ...
 if (caller != ((void *)0))
 {
  ...
  do { unsigned long* __ebp = 0;
 unsigned long* __old_ebp = 0;
 unsigned long __eip = 0;
 int __i = 0;
 long* __p = (long*)(g_glibc_fbt);
 for (;
 __i < 12;
 __i++) { if (__i == 0) { __asm__ volatile ("movl $., %0" : "=r" (__eip));
 *__p++ = __eip;
 continue;
 } else if (__i == 1) { __asm__ volatile ("mov %%ebp, %0" : "=r" (__ebp));
 } else { __old_ebp = __ebp;
 __asm__ volatile ("mov %1, %%eax" :"=r"(__ebp));
 if ((__ebp >= __old_ebp) || (__old_ebp > __ebp + 2560)) { *__p++ = 0xdeadbeaf;
 break;
 } } if (__ebp == 0) break;
 *__p++ = *(__ebp + 1);
 } *__p = 0;
  ...
  fprintf(mallstream, " |0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x| ",
     ...
     const void * caller;
{
  ...
  __malloc_hook = tr_old_malloc_hook;
  if (tr_old_memalign_hook != ((void *)0))
...
smtrace ()
{
  ...
  if (mallfile != ((void *)0))
  {
   ...
   mallstream = fopen (mallfile != ((void *)0) ? mallfile : "/dev/null", "wc");
   if (mallstream != ((void *)0))
   {

    int flags = fcntl (fileno (mallstream), 1, 0);
    if (flags >= 0)
    {
     flags |= 1;
     fcntl (fileno (mallstream), 2, flags);
    }

    malloc_trace_buffer = mtb;
    setvbuf (mallstream, malloc_trace_buffer, 0, 512);
